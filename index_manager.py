...
class IndexShardManager:
    ...
    def load_indices(self):
        ...
        if not valid_files:
            ...
            for suffix_id, filepath in valid_files:
                ...
                if self.index_type == "binary":
                    idx = faiss.read_index_binary(filepath)
                else:
    ...
    def save_active_index(self):
        ...
        if self.index_type == "binary":
            faiss.write_index_binary(self.active_index, filename)
        else:
            faiss.write_index(self.active_index, filename)
    ...
    def add_batch(self, vectors, image_paths_list):
        for vec, path in zip(vectors, image_paths_list):
            self.add(vec, path)
        print(f"Added batch: {len(image_paths_list)} vectors to {self.prefix}")
    ...
    def search_with_paths(self, query_vector, k=1):
        results = self.search(query_vector, k)
        
        results_with_paths = []
        for dist, global_idx in results:
            path = self.paths[global_idx]
            results_with_paths.append((dist, path))
    ...
    def get_shard_info(self):
        ...
        for idx_obj, suffix_id in self.indices:
            info.append({
                ...
                "filename": self.get_index_filename(suffix_id)
            })
        return info
